...
export default function Home() {
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white">
      <header className="sticky top-0 z-30 border-b border-white/10 bg-[#0A0A0A]/95 backdrop-blur">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
          <div>
            ...
            <p className="text-lg font-black tracking-[0.18em] sm:text-xl">CAR SHOW</p>
          </div>
      ...
      <main className="mx-auto flex w-full max-w-7xl flex-col gap-7 px-4 py-5 sm:gap-9 sm:px-6 sm:py-8 lg:gap-12 lg:px-8 lg:py-12">
        ...
        <section id="categorias" className="space-y-3">
          <h2 className="text-xl font-bold sm:text-2xl">Categorías destacadas</h2>
          <div className={`${carouselBase} md:grid md:grid-cols-3 md:gap-3 md:overflow-visible md:pb-0`}>
        ...
        <section id="productos" className="space-y-3">
          <div className="flex items-end justify-between gap-3">
            ...
            <a href="#" className="text-xs text-[#ff8b87] hover:text-[#E10600] sm:text-sm">
              Ver catálogo completo
            </a>
          </div>
          <div className={`${carouselBase} md:grid md:grid-cols-3 md:gap-4 md:overflow-visible md:pb-0`}>
            {products.map((product) => (
              ...
              >
                ...
                <div className="mt-3 space-y-1.5">
                  ...
                  <p className="text-lg font-extrabold text-white sm:text-xl">{product.price}</p>
                  <span
        ...
        <section id="mas-vendidos" className="space-y-3">
          <div className="flex items-end justify-between gap-3">
            ...
            <span className="text-xs text-zinc-400 sm:text-sm">Top consultados</span>
          </div>
          <div className={`${carouselBase} lg:grid lg:grid-cols-4 lg:gap-4 lg:overflow-visible lg:pb-0`}>
            {topSellers.map((product) => (
              ...
              >
                <p className="text-[11px] uppercase tracking-widest text-zinc-400">{product.category}</p>
                <h3 className="mt-1.5 text-base font-semibold">{product.name}</h3>
                <p className="mt-2 text-lg font-extrabold text-[#ff8b87]">{product.price}</p>
                <a
                  href="https://wa.me/595981000000"
                  target="_blank"
                  rel="noreferrer"
        ...
        >
          <h2 className="text-xl font-bold sm:text-2xl">Promociones destacadas</h2>
          <div className={`${carouselBase} md:grid md:grid-cols-3 md:gap-4 md:overflow-visible md:pb-0`}>
            {promos.map((promo) => (
              <article
                key={promo.title}
              ...
              >
                ...
                <h3 className="mt-1.5 text-base font-bold sm:text-lg">{promo.title}</h3>
        ...
        <section className="rounded-2xl border border-[#FFD700]/40 bg-[#1a1703] p-4 sm:p-6 lg:p-8">
        ...
        <section id="combos" className="space-y-3">
          <h2 className="text-xl font-bold sm:text-2xl">Combos recomendados</h2>
          <div className={`${carouselBase} md:grid md:grid-cols-3 md:gap-4 md:overflow-visible md:pb-0`}>
            {combos.map((combo) => (
              <article
                key={combo.name}
              ...
              >
                <h3 className="text-base font-bold sm:text-lg">{combo.name}</h3>
                <p className="mt-1.5 text-sm text-zinc-300">{combo.detail}</p>
                <p className="mt-3 text-base font-extrabold text-[#ff8b87]">{combo.price}</p>
              </article>
            ))}
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-white/10 bg-zinc-900 p-4 sm:p-6 lg:p-8">
          <h2 className="text-xl font-bold sm:text-2xl">¿Por qué elegir CAR SHOW?</h2>
